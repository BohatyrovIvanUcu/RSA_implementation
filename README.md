# 🔐 Безпечний термінальний чат з RSA шифруванням

## Інструкції до запуску

### Вимоги
- Python 3.8+
- Додаткові бібліотеки **не потрібні** (використовується лише стандартна бібліотека)

### Запуск

Потрібно відкрити **3 термінали** (PowerShell або CMD):

**Термінал 1 — Запуск сервера:**
```bash
cd "RSA implementation"
python server.py
```

**Термінал 2 — Запуск першого клієнта:**
```bash
cd "RSA implementation"
python client.py
```
> Введіть ім'я користувача (наприклад, `Alice Smith`)

**Термінал 3 — Запуск другого клієнта:**
```bash
cd "RSA implementation"
python client.py
```
> Введіть ім'я користувача (наприклад, `John Smith`)

Після підключення обох клієнтів, просто пишіть повідомлення — вони автоматично шифруються та перевіряються на цілісність.

---

## Коротке пояснення імплементації

### Структура проєкту

| Файл | Опис |
|------|------|
| `rsa_crypto.py` | Модуль криптографії: RSA, симетричне шифрування, хешування |
| `server.py` | Чат-сервер з обміном ключами та шифруванням |
| `client.py` | Чат-клієнт з обміном ключами та шифруванням |

### Алгоритм RSA (реалізований з нуля, без сторонніх бібліотек)

#### Генерація ключів:
1. Генеруємо два великих випадкових простих числа `p` і `q` (по 512 біт кожне)
2. Обчислюємо модуль: `n = p × q`
3. Обчислюємо функцію Ейлера: `φ(n) = (p-1)(q-1)`
4. Обираємо публічну експоненту: `e = 65537`
5. Обчислюємо приватну експоненту: `d = e⁻¹ mod φ(n)` (через розширений алгоритм Евкліда)

**Публічний ключ:** `(e, n)` — можна передавати відкрито
**Приватний ключ:** `(d, n)` — зберігається в секреті

#### Шифрування / Дешифрування:
- **Шифрування:** `c = m^e mod n` (де `m` — числове представлення повідомлення)
- **Дешифрування:** `m = c^d mod n`

#### Перевірка простоти:
Використовується **тест Міллера-Рабіна** з 20 раундами (ймовірність помилки < 10⁻¹²)

### Обмін ключами (Key Exchange)

```mermaid
sequenceDiagram
    participant C as Клієнт
    participant S as Сервер

    C->>S: username (відкритий текст)
    S->>C: server_public_key (e, n)
    C->>S: client_public_key (e, n)

    Note right of S: Генерує випадковий<br/>симетричний ключ (32 байти)<br/>та шифрує його RSA-ключем клієнта

    S->>C: encrypted_symmetric_key

    Note left of C: Розшифровує своїм<br/>приватним RSA-ключем

    Note over C,S: Спільний секрет встановлено
```

### Шифрування повідомлень

Після обміну ключами, всі повідомлення шифруються **симетричним XOR-шифром** з використанням спільного секретного ключа (32 байти).

### Перевірка цілісності (Message Integrity)

Для кожного повідомлення використовується **SHA-256** хешування:

```mermaid
sequenceDiagram
    participant A as Відправник
    participant B as Отримувач

    Note left of A: 1. hash = SHA256(plaintext)<br/>2. encrypted = encrypt(plaintext)

    A->>B: (hash, encrypted_message)

    Note right of B: 1. plaintext = decrypt(encrypted)<br/>2. new_hash = SHA256(plaintext)<br/>3. Порівняти hash == new_hash

    alt Хеші співпадають
        Note right of B: Повідомлення цілісне ✓
    else Хеші різні
        Note right of B: Повідомлення підроблене ✗
    end
```

Це гарантує, що якщо зашифроване повідомлення було змінено під час передачі, отримувач це виявить.

---

## Результати запусків

### Запуск сервера
Сервер генерує RSA ключі та чекає на підключення:
```
[server] Генерація RSA ключів (може зайняти кілька секунд)...
[server] RSA ключі згенеровано!
[server] Публічний ключ: e=65537, n=(1024 біт)
[server] Очікування підключень на 127.0.0.1:9001...
```

### Підключення клієнта
Клієнт генерує свої ключі та виконує обмін:
```
[client] Генерація RSA ключів...
[client] RSA ключі згенеровано!
[client] Публічний ключ сервера отримано
[client] Наш публічний ключ надіслано серверу
[client] Симетричний ключ отримано та розшифровано (32 байт)
[client] ✓ Безпечне з'єднання встановлено!
[client] Можете починати спілкування:
```

### Сервер при підключенні клієнта
```
[server] Alice намагається підключитися з ('127.0.0.1', 54321)
[server] Публічний ключ надіслано → Alice
[server] Публічний ключ отримано ← Alice
[server] Симетричний ключ згенеровано для Alice (32 байт)
[server] Зашифрований симетричний ключ надіслано → Alice
[server] ✓ Безпечне з'єднання встановлено з Alice!
```

### Обмін повідомленнями
**Alice пише:**
```
Привіт, Bob!
```

**На сервері:**
```
[server] Повідомлення від Alice: цілісність OK ✓
```

**Bob отримує:**
```
Alice: Привіт, Bob!
```

---

## Матеріали

- [EverythingCrypto — збірка теорії по криптографії](https://github.com/zademn/EverythingCrypto)
- [Cryptography with Python — Hashing](https://medium.com/@ashiqgiga07/cryptography-with-python-hashing-d0b7dbf7767)
